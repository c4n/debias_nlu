...
def report_CMA(
    ...
    for seed_idx in range(len(seed_path)):
        ...
        for i in range(len(labels)):
            ...
            pp=p[0]/max(p[1:2])
            bb=b[0]/max(b[1:2])
            ana_tie.append(pp/bb-1)
            
            ### TIEception
            constant_x0 = (1/n_labels) * np.ones(n_labels)
            new_ya1x0=fusion(np.array(a1[i]), constant_x0)
            # new_x1 = softmax(TIE)
            new_te = np.array(x1[i] - te_correction*a1[i])
            new_x1 = softmax(new_te)
            new_ya1x1 =  fusion(np.array(a1[i]), new_x1)
            p2=softmax(new_ya1x1)
            b2=softmax(new_ya1x0)
            pp2=p2[0]/max(p2[1:2])
            bb2=b2[0]/max(b2[1:2])
            ana_tie2.append(pp2/bb2-1)
            ...
            if entropy > entropy_threshold:
                ...
                all_cls_my_causal_raw.append(np.array(x1[i] - te_correction*a1[i]))
        ...
        raw_TIE_correct.append(TIE_pred_correct)
        raw_TE_correct.append(pred_correct)
        raw_TIE.append(all_TIE)
        raw_ana_TIE.append(ana_tie)
        raw_ana_TIE2.append(ana_tie2)
        raw_entropy.append(temp_entropy)    
    ...
    print("normal-TIE-TE")
    print(np.array(factual_scores).mean(),np.array(TIE_scores).mean(),np.array(my_causal_query).mean())


    
    if return_raw:
        # Return raw values
        return raw_factual_correct, raw_TIE, raw_ana_TIE,raw_ana_TIE2,raw_TIE_correct, raw_TE_correct