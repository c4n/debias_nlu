...
@Trainer.register("gradient_descent_kl_hypo", constructor="from_partial_objects")
class GradientDescentKLHypoTrainer(GradientDescentTrainer):
    ...
    @overrides                         
    def batch_outputs(self, batch: TensorDict, for_training: bool, kl : bool = False) -> Dict[str, torch.Tensor]:
        """
        ...
        """
        output_dict = self._pytorch_model(**batch, kl=kl)
        ...
        for batch_group, adversarial_batch_group in batch_group_generator_tqdm:
            ...
            for batch, adversarial_batch in zip(batch_group,adversarial_batch_group):
                ...
                with amp.autocast(self._use_amp):
                    batch_outputs = self.batch_outputs(batch, for_training=True, kl=False) # Normal backward pass
                ...
                #Reverse Gradient
                with amp.autocast(self._use_amp):
                    adversarial_batch_outputs = self.batch_outputs(adversarial_batch, for_training=True, kl=True) #grad reverse backward pass
