...
class CounterfactualTextualEntailmentPredictor(Predictor):
    ...
    def predict_batch_instance(
        ...
        for i in range(len(outputs)):
            ...
            outputs[i]["cf_logits"] = cf_outputs[i]["logits"]
            n_class = outputs[i]["logits"].shape[0]
            factual_softmax = softmax(outputs[i]["logits"])
            counterfactual_softmax = softmax(cf_outputs[i]["logits"])
            outputs[i]["probs"] = factual_softmax 
            outputs[i]["cf_probs"] = counterfactual_softmax
