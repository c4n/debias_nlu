...
@Predictor.register("cf_textual_entailment_distill")
class CounterfactualTextualEntailmentPredictor(Predictor):
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        ...
        if 'gold_label'  in json_dict.keys() and 'sample_weight' in json_dict.keys():  
            label = json_dict["gold_label"]      
            sample_weight = json_dict["sample_weight"]
            return self._dataset_reader.text_to_instance(
                premise_text, hypothesis_text, label=label, sample_weight=sample_weight
            )
        elif 'gold_label' in json_dict.keys():
            label = json_dict["gold_label"] 
            return self._dataset_reader.text_to_instance(
                premise_text, hypothesis_text, label=label
            )            

        elif 'sample_weight' in json_dict.keys():
            sample_weight = json_dict["sample_weight"]
            return self._dataset_reader.text_to_instance(
    ...
    def predict_batch_instance(
        ...
        # inv_label_dict = {'entailment':0,'neutral':2,'contradiction':1}
        gt = []
        ...
        for i in range(len(outputs)):
            ...
            if gt[i]=='entailment':
                outputs[i]['logits'] = outputs[i]['logits']# -  ((factual_entropy**entropy_curve)  * cf_weight * cf_outputs[i]['logits'])
            else:
                outputs[i]['logits'] = outputs[i]['logits']  -  ((factual_entropy**entropy_curve)  * cf_weight * cf_outputs[i]['logits'])               
