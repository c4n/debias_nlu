...
@Model.register("adv_fc_classifier")
class AdvFcClassifier(BasicClassifier):
    def __init__(
        self,
        vocab: Vocabulary,
        ...
        **kwargs,
        ...
        self._kl_loss = torch.nn.KLDivLoss(reduction='batchmean')
        initializer(self)    
        
        
    def forward(  # type: ignore
        self, tokens: TextFieldTensors, label: torch.IntTensor = None, kl: bool = False
        ...
        if label is not None:
            if kl == True:
                unif_dist = torch.ones_like(probs) * 1.0 / 3.0
                loss = self._kl_loss(probs, unif_dist)
            else:
