...
from my_package.data.dataset_readers.counterfactual_reader_hypo import CounterfactualSnliHypoReader
...
@Subcommand.register("cf_predict")
...
class _CFPredictManager:
    ...
    def _predict_json(self, batch_data: List[JsonDict], cf_weight : float) -> Iterator[str]:
        if len(batch_data) == 1:
            results = [self._predictor.predict_json(batch_data[0],cf_weight)]
        else:
            results = self._predictor.predict_batch_json(batch_data,cf_weight)
    ...
    def run(self) -> None:
        ...
        if has_reader:
            for batch in lazy_groups_of(self._get_instance_data(), self._batch_size):
                for model_input_instance, result in zip(batch, self._predict_instances(batch, self._cf_weight)):
