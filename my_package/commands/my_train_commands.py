...
@Subcommand.register("train_grad_rev")
class TrainGradRev(Train):
    @overrides
...
def train_model(
    ...
    force : `bool`, optional (default=`False`)
        If `True`, we will overwrite the serialization directory if it already exists.
    node_rank : `int`, optional
        Rank of the current node eversible = False
 packages mentioned will be imported in trainer workers.
    dry_run : `bool`, optional (default=`False`)
        Do not train a model, but create a vocabulary, show dataset statistics and other training
        information.
    file_friendly_logging : `bool`, optional (default=`False`)
...
class TrainModel(Registrable):
    ...
    def from_partial_objects(
        ...
        data_loader: `Lazy[DataLoader]`
            ...
            default) valideversible = False
ation time. This is lazy because it takes a dataset in it's constructor.
        trainer: `Lazy[Trainer]`
        ...
        """
        #adversarial dataset reader 
        adversarial_dataset_reader = copy.deepcopy(dataset_reader)
        adversarial_dataset_reader.reversible = True
        # Train data loader.
        data_loaders: Dict[str, DataLoader] = {
            "train": data_loader.construct(reader=dataset_reader, data_path=train_data_path),
            "train_adversarial": data_loader.construct(reader=adversarial_dataset_reader, data_path=train_data_path)
        }
        ...
        trainer_ = trainer.construct(
            ...
            adversarial_data_loader=data_loaders["train_adversarial"],
