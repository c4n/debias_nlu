...
def get_c(
    ...
    df_bias_dev = pd.read_json(
        os.path.join(data_path, bias_val_pred_file),
        lines=True
    )
    ...
    df_bert_dev = pd.read_json(
        os.path.join(model_path, model_val_pred_file),
        lines=True
    )
    ya1x1prob_dev = []
...
def report_CMA(
    ...
    for seed_idx in range(len(seed_path)):
        ...
        for x_f1, x_y_preds in zip(
            ...
            f1_scores = f1_score(
                y_true=labels, y_pred=x_y_preds,
    ...
    print('factual score:')
    print(factual_scores, np.array(factual_scores).mean(),
          np.array(factual_scores).std())
    print("TE:")
    ...
    print("TIE F1:", TIE_f1, {"%s_mean_sd" % k: [np.mean(v), np.std(v)]
          for k, v in TIE_f1.items()})
    print("NIE:")
    ...
    print("INTmed:")
    print(INTmed_explain, np.array(INTmed_explain).mean(),
          np.array(INTmed_explain).std())
    print("INTmed acc:")
    print(INTmed_scores, np.array(INTmed_scores).mean(),
          np.array(INTmed_scores).std())
    print("my query:")
    print(my_causal_query, np.array(my_causal_query).mean(),
          np.array(my_causal_query).std())
    print("my query F1:", my_causal_f1, {
